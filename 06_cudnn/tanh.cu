...
int main(){
    const int batch_size = 32; // NCHW format
    const int channels = 32;
    const int height = 224;
    const int width = 224;
    const int tensor_size = batch_size * channels * height * width;

    float *h_input, *h_output_cpu ,*h_output_naive , *h_output_cudnn;
    ...
    CHECK_CUDNN(cudnnSetActivationDescriptor(act_desc, CUDNN_ACTIVATION_TANH, CUDNN_PROPAGATE_NAN, 0.0));

    float alpha = 1.0f, beta = 0.0f;

    // Warmup runs for cuDNN
    for (int i = 0; i < num_warmup; ++i) {
        CHECK_CUDNN(cudnnActivationForward(handle, act_desc, &alpha, input_desc, d_input , &beta, input_desc, d_output_cudnn));
    }
    CHECK_CUDA(cudaDeviceSynchronize());
