// note that in cublaslt when we have large matrices always the m and k must be in multiples of 4
//Tensor Cores / FP16 kernels
...
int main(){
    ...
    CHECK_CUDA(cudaMalloc(&d_A_fp32, sizeof(float) * M * K));
    CHECK_CUDA(cudaMalloc(&d_B_fp32, sizeof(float) * K * N));
    CHECK_CUDA(cudaMalloc(&d_C_fp32, sizeof(float) * M * N));


    half *d_A_fp16, *d_B_fp16, *d_C_fp16;
    CHECK_CUDA(cudaMalloc(&d_A_fp16, sizeof(half) * M * K));
    CHECK_CUDA(cudaMalloc(&d_B_fp16, sizeof(half) * K * N));
    CHECK_CUDA(cudaMalloc(&d_C_fp16, sizeof(half) * M * N));

    CHECK_CUDA(cudaMemcpy(d_A_fp32, h_A, sizeof(float) * M * K, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaMemcpy(d_B_fp32, h_B, sizeof(float) * K * N, cudaMemcpyHostToDevice));
    ...
    //std::vector<half> is an object so we have to send like the pointer to it so we send h_A_half.data()
    //but when we initalize like h_A alone it is static not an object .... h_A behaves like &h_A[0];
    ...
    //setting up the matrix descriptor
    ...
    //settiing up the matrix multiplication descriptor
    ...
    //set how cublasLt should interpret A and B
    ...
    CHECK_CUBLAS(cublasLtMatmul(handle,matmulDesc_fp16,&alpha_h,d_B_fp16,matB_fp16,d_A_fp16,matA_fp16,&beta_h,d_C_fp16,matC_fp16,d_C_fp16,matC_fp16,nullptr,nullptr,0,0));


    CHECK_CUDA(cudaMemcpy(h_C_gpu_fp32,d_C_fp32,sizeof(float)*M*N,cudaMemcpyDeviceToHost));
